...
public class Heap {
    ...
    public void decreaseKey(int index, double value) {
        if (value < this.heapArray.get(index)) {
            this.heapArray.set(index, value);
            int parent_index = (index + 1) / 2 - 1;
            while (index > 0 && this.heapArray.get(parent_index) > value) {
                double buf = this.heapArray.remove(parent_index);
                this.heapArray.add(parent_index, value);
